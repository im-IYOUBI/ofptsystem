...
@section('content')
    <div class="roles">
        ...
        <div class="table w-full mt-8 bg-white rounded shadow-md">
            <form action="{{ route('student.store') }}" method="POST" class="w-full max-w-2xl px-8 py-10 mx-auto" enctype="multipart/form-data">
                @csrf
                ...
                @php
                    $formFields = [
                        ...
                        ['label' => 'Roll Number', 'type' => 'number', 'name' => 'roll_number', 'value' => old('roll_number')],
                        ['label' => 'Phone', 'type' => 'text', 'name' => 'phone', 'value' => old('phone')],
                        ['label' => 'Date of Birth', 'type' => 'text', 'name' => 'dateofbirth', 'value' => old('dateofbirth'), 'additional' => 'id="datepicker-sc" autocomplete="off"']
                    ];
                @endphp
                ...
                @php
                    $selectFields = [
                        ['label' => 'Current Address', 'name' => 'current_address', 'value' => old('current_address')],
                        ['label' => 'Permanent Address', 'name' => 'permanent_address', 'value' => old('permanent_address')],
                        ['label' => 'Assign Class', 'name' => 'class_id', 'options' => $classes, 'optionLabel' => 'class_name'],
                        ['label' => 'Student\'s Parent', 'name' => 'parent_id', 'options' => $parents, 'optionLabel' => 'user.name']
                    ];
                @endphp
                        ...
                        @if(isset($field['options']))
                            <select name="{{ $field['name'] }}" class="block appearance-none w-full bg-gray-100 border border-gray-300 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500">
                                ...
                                @foreach ($field['options'] as $option)
                                    <option value="{{ $option->id }}">{{ data_get($option, $field['optionLabel']) }}</option>
                                @endforeach
                            </select>
                        @else
