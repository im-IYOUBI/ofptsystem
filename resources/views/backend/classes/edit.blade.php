...
@section('content')
    <div class="roles">
        ...
        <div class="table w-full mt-8 bg-white rounded shadow-md">
            <form action="{{ route('classes.update', $class->id) }}" method="POST" class="w-full max-w-2xl px-8 py-10 mx-auto">
                @csrf
                @method('PUT')
                
                @php
                    $formFields = [
                        ['label' => 'Class Name', 'type' => 'text', 'name' => 'class_name', 'value' => $class->class_name],
                        ['label' => 'Class Numeric', 'type' => 'number', 'name' => 'class_numeric', 'value' => $class->class_numeric],
                        ['label' => 'Class Description', 'type' => 'text', 'name' => 'class_description', 'value' => $class->class_description],
                    ];
                @endphp
