                ...
                <div class="flex flex-wrap items-center text-gray-700 border-t-2 border-l-4 border-r-4 border-gray-300">
                    ...
                    <div class="w-2/12 px-4 py-3 flex flex-wrap">
                        @if ($user->roles->isEmpty())
                            <span class="bg-red-200 text-xs font-semibold text-red-600 tracking-tight px-3 py-1 border rounded-full">NO ROLE ASSIGNED YET!</span>
                        @else
                            @foreach ($user->roles as $role)
                                <span class="bg-gray-200 text-xs font-semibold text-gray-600 tracking-tight px-3 py-1 border rounded-full">
                                    @if ($role->name == 'Trainer')
                                        Trainer
                                    @elseif ($role->name == 'Stagaire')
                                        Stagaire
                                    @elseif ($role->name == 'Module')
                                        Module
                                    @else
                                        {{ $role->name }}
                                    @endif
