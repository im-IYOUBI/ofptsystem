    ...
    <div class="roles">
        ...
        <div class="flex items-center justify-between mb-6">
            ...
            <div class="flex flex-wrap items-center">
                <a href="{{ route('subject.index') }}" class="bg-gray-700 text-white text-sm uppercase py-2 px-4 flex items-center rounded hover:bg-gray-600 transition duration-300">
        ...
        <div class="table w-full mt-8 bg-white rounded shadow-md">
            <form action="{{ route('subject.update', $subject->id) }}" method="POST" class="w-full max-w-2xl px-8 py-10 mx-auto">
                @csrf
                @method('PUT')

                @php
                    $formFields = [
                        ['label' => 'Subject Name', 'type' => 'text', 'name' => 'name', 'value' => $subject->name],
                        ['label' => 'Subject Code', 'type' => 'number', 'name' => 'subject_code', 'value' => $subject->subject_code],
                        ['label' => 'Subject Description', 'type' => 'text', 'name' => 'description', 'value' => $subject->description],
                    ];
                @endphp
