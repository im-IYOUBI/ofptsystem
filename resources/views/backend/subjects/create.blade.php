...
@section('content')
    <div class="roles">
        ...
        <div class="table w-full mt-8 bg-white rounded shadow-md">
            <form action="{{ route('subject.store') }}" method="POST" class="w-full max-w-2xl px-8 py-10 mx-auto">
                @csrf
                @php
                    $formFields = [
                        ['label' => 'Subject Name', 'type' => 'text', 'name' => 'name', 'value' => old('name')],
                        ['label' => 'Subject Code', 'type' => 'number', 'name' => 'subject_code', 'value' => old('subject_code')],
                        ['label' => 'Subject Description', 'type' => 'text', 'name' => 'description', 'value' => old('description')],
                    ];
                @endphp
